// Stages list
const stages = [
  "A0-G1", "G1 Stage", "A1-G2", "G2 Stage", "A2-G3", "G3 Stage",
  "A3-G4", "G4 Stage", "A4-G5", "G5 Stage", "A5-G6", "G6 Stage"
];
...
export const createGanttTasks = (templateTasks, plannedDates, actualTasks) => {
  ...
  const getPlannedDatesForTask = (taskName) => {
    ...
    const matchingPlannedDate = plannedDates.records.find(record =>
      record[`${taskKey}_Planned_Start_Date__c`] && record[`${taskKey}_Planned_End_Date__c`]
    );
  ...
  const getActualTaskDetails = (taskName) => {
    const taskKey = taskName.substring(0, 5);
    const matchingTask = actualTasks.records.find(task => task.Subject.startsWith(taskKey + "-"));
    ...
    if (matchingTask) {
      ...
      const expectedEndDate = new Date(startDate.getTime() + daysToAdd * 24 * 60 * 60 * 1000);
      ...
      if (matchingTask.Status === "In Progress") {
        return {
          ...
          completedDate: matchingTask.CompletedDateTime ? new Date(matchingTask.CompletedDateTime) : null,
  ...
  const calculateProgress = (startDate, endDate, currentDate, completedDate, status, overdueFlag, expectedEndDate) => {
    ...
    if (status === "In Progress") {
      ...
      return Math.min(Math.max(100 - (remainingDuration / totalDuration) * 100, 0), 100);
    }
    return 0;
  };
  ...
  const getTaskStyles = (status, completedDate, endDate, overdueFlag) => {
    ...
    if (status === "In Progress") {
      return { progressColor: overdueFlag ? "#f76876" : "orange", backgroundColor: "orange" };
    }
  ...
  stages.forEach((stage) => {
    ...
    stageTasks.forEach(task => {
      ...
      if (actualTaskDetails || plannedTaskDates) {
        const startDate = new Date(actualTaskDetails?.start || plannedTaskDates?.start);
        const endDate = new Date(actualTaskDetails?.end || plannedTaskDates?.end);
        ...
        const { progressColor, backgroundColor } = getTaskStyles(status, completedDate, endDate, overdueFlag);

        // Create task object and add to Gantt tasks
        ganttTasks.push({
          start: startDate,
          end: status === "Completed" && completedDate ? completedDate : expectedEndDate || plannedDates.end,
    ...
    stageDates[stage] = {
      start: projectStartDate || new Date(currentDate.getFullYear(), currentDate.getMonth(), 10),
      end: projectEndDate || new Date(currentDate.getFullYear(), currentDate.getMonth(), 15)
    };
  ...
  // Second pass: Adjust dates for specific "G" stages based on dependencies
  stages.forEach((stage, stageIndex) => {
    const projectIndex = ganttTasks.findIndex(task => task.name === stage);

    // Apply specific date logic for "G" stages
    if (stage.startsWith("G") && stageIndex > 0 && stageIndex < stages.length - 1) {
      const prevStage = stages[stageIndex - 1];
      const nextStage = stages[stageIndex + 1];

      // Adjust dates only if both preceding and following stages have been populated in `stageDates`
